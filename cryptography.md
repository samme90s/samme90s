...
Hashing is a technique used to generate a unique value for a given input, commonly employed to detect any alterations in the input. This concept is analogous to a wax seal used in medieval times. An example of a hash is a checksum, providing a summation of all the bytes in a file; if the file is changed, the checksum will differ. While this is a straightforward method, it can be easily compromised. A more secure approach involves using a cryptographic hash function such as _SHA_ (Secure Hash Algorithm). _SHA_ is a one-way function, meaning it is computationally easy to produce a hash from a given input but exceedingly challenging to generate an input that would yield a specific hash. This irreversibility is by design. The hash function accepts an input of any size and returns a fixed-length string. If the input is altered, the hash will be distinct. This highly secure method finds applications in various fields, including password storage. The hashing technique produces a _hash digest_.

### Digital Signatures

Digital signatures serve to verify the authenticity and _integrity_ of a message. Once a signature is attached to a message, it becomes _unalterable_ by either party (sender or receiver). The immutability of the signature is achieved through complex mathematical operations that are computationally secure. These signatures are generated using the private key in an asymmetric key pair. Subsequently, the signature is **not** decrypted but rather verified using the corresponding public key. The verification process involves applying mathematical operations to the signature, mirroring the operations performed with the private key during the signing process. This ensures that the signature is valid, confirming the integrity of the original message. One example of a digital signature is the _RSA_ signature scheme.
